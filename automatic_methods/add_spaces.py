...
with open('E:\\Python_Calismalari\\fasttext\\mutated_tweets\\add_spaces\\'+tasks[index]+'\\'+str(addTime)+'_test_preprocessed.csv', mode='w', newline='') as csv_file:
    fieldnames = ['Tweet', 'Stance', 'Index', 'Original Tweet']
    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
    with open('E:\\Python_Calismalari\\fasttext\\mutated_tweets\\add_spaces\\'+tasks[index]+'\\test_preprocessed.csv') as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                char_index = 0
                k = 0
                word_list = newTweet.split()#re.findall(r'\S+', newTweet)
                p_list = []
                kare_start = 0
                ...
                for i in sorted_x:
                    if (k < addTime):
                        if(len(word_list[i[0]])>=3):
                            leter = random.randint(1, len(word_list[i[0]])-1)
                            word_list[i[0]] = word_list[i[0]][:int(leter)] + ' ' + word_list[i[0]][int(leter):]
                            k = k + 1
                    else:
                        break
